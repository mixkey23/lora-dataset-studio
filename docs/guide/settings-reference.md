...
A general-purpose, local peer of **Klein** inside **Generate variations** — same shot catalog, same NSFW lane, same fan-out mechanics, but running **Qwen-Image-Edit-2511** instead of Klein's FLUX.2-Kontext lineage, via a VLM-driven edit workflow (a `TextEncodeQwenImageEditPlusCustom` node reads the reference image and the plain-language shot instruction together, rather than a hand-built "create a new X of the same Y" prompt). It's useful when a dataset's **render style** (3D / anime / cartoon / illustration — see *Per-dataset settings* below) fights Klein's own photoreal-biased fine-tune; Qwen-Image-Edit-2511 has no equivalent documented bias, though this is not yet validated against real output. It shares its UNET/VAE/text-encoder with **Qwen Multi-angle** above — point ComfyUI at the same checkpoint once and both engines pick it up automatically; there's no separate model path to configure here. Unlike Klein (and Qwen Multi-angle), there's **no consistency-LoRA setting** — that LoRA exists to fight drift across several reference images, and this engine always starts from one (Character datasets), so it isn't part of its workflow. If you keep several Qwen-Image-Edit-2511 checkpoints side by side (e.g. an SFW build and an uncensored community fine-tune), a **Base model** picker appears in the workspace's 🎨 Qwen Edit tuning panel — same shape as Klein's own picker, hidden entirely when only one checkpoint is found. Stored in `qwen_edit.*`:
