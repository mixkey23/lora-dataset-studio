...
This is where you connect the services that *generate* dataset images. **Generate variations** offers six generator engines: **Nano Banana** (Google Gemini), **ChatGPT** (OpenAI), **OpenRouter** (one account in front of many providers), and three that run locally through ComfyUI — **Klein**, **Krea 2 Edit** and **Qwen Edit** (Qwen-Image-Edit-2511). Each of the three API engines lets you choose which model it asks for — see *Image models* below. ComfyUI itself is configured under **Local tools**; the API keys and the three local engines' own tuning cards are configured here. A separate, seventh local engine, **Qwen Multi-angle**, doesn't generate new dataset images from a reference — it rotates the camera angle of an image you already have, and is configured below.
...
- **Default engine** → `engines.default`. Which engine is preselected in the workspace. One of `nanobanana`, `chatgpt`, `openrouter`, `klein`, `krea`, `qwen_edit`. Default **`chatgpt`**.
...
A general-purpose, local peer of **Klein** inside **Generate variations** — same shot catalog, same fan-out mechanics, but running **Qwen-Image-Edit-2511** instead of Klein's FLUX.2-Kontext lineage, via a VLM-driven edit workflow (a `TextEncodeQwenImageEditPlusCustom` node reads the reference image and the plain-language shot instruction together, rather than a hand-built "create a new X of the same Y" prompt). It's useful when a dataset's **render style** (3D / anime / cartoon / illustration — see *Per-dataset settings* below) fights Klein's own photoreal-biased fine-tune; Qwen-Image-Edit-2511 has no equivalent documented bias, though this is not yet validated against real output. It shares its UNET/VAE/text-encoder with **Qwen Multi-angle** above — point ComfyUI at the same checkpoint once and both engines pick it up automatically; there's no separate model path to configure here for SFW shots. Unlike Klein (and Qwen Multi-angle), there's **no consistency-LoRA setting** — that LoRA exists to fight drift across several reference images, and this engine always starts from one (Character datasets), so it isn't part of its workflow. If you keep several Qwen-Image-Edit-2511 checkpoints side by side, a **Base model** picker appears in the workspace's 🎨 Qwen Edit tuning panel — same shape as Klein's own picker, hidden entirely when only one checkpoint is found. Stored in `qwen_edit.*`:

- **Lightning speed LoRA** → `qwen_edit.lightning_enabled` (default **on**) + `qwen_edit.lightning_strength` (`0`–`1.5`, default **`1.0`**). When on, sampling runs the LoRA's fast 8-step/cfg-1 envelope; when off (or the file is missing), sampling falls back to a slower, non-distilled envelope — both step counts are extrapolated, not yet measured against a real run.

**NSFW is NOT just a prompt toggle here, unlike Klein.** The base (SFW) checkpoint above has baked-in refusal that a prompt alone can't steer around — NSFW shots run on a genuinely different graph, using [Phr00t/Qwen-Image-Edit-Rapid-AIO](https://huggingface.co/Phr00t/Qwen-Image-Edit-Rapid-AIO) (a merged, single-file checkpoint that ships a dedicated NSFW build from v5 onward). Drop that checkpoint anywhere under a ComfyUI `checkpoints` folder and it's picked up automatically — no config path to set. Until it's there, NSFW shots on Qwen Edit fail with an actionable message instead of silently generating on the SFW checkpoint (which would likely refuse or distort the request). This checkpoint has its own simpler workflow (no Lightning toggle — its acceleration is baked in; fixed 768×768 output, not aspect-ratio-aware like the SFW path).

The engine only lights up (`caps.engines.qwen_edit`) once the UNET, VAE and text-encoder are found on disk (the Lightning LoRA is quality-only and degrades gracefully when absent). The output's aspect ratio always matches the reference image (computed dynamically, not fixed). Its negative prompt is built automatically from the dataset's render style (anti-photorealism terms for a stylized target, generic quality terms otherwise) — there's nothing to configure for it. NSFW shots on Qwen Edit never leave your machine, exactly like Klein.
...
- **Qwen Edit — restage & face-identity block** → `identity_prompts.qwen_edit_identity`. The same instruction block for the local **Qwen Edit** engine.
...
musubi-tuner runs can also generate sample preview images during training (same prompts as ai-toolkit's own sample block, same **Preview images** toggle — see *Advanced options (per run)* below), so the Runs-hub card shows a real thumbnail instead of the generic family tile. Previews render at the training resolution capped at 1024px (never upscaled past a profile's own smaller resolution) — sampling at the full 1328px some GPU profiles train at added a VAE decode spike that could OOM a card already near its budget (reported on a 32GB card using the `rtx5090` profile) without changing the actual training recipe at all.
...
- **Default training family** → `training.default_family`. The model family preselected when you start a new run. One of `zimage`, `sdxl`, `krea`, `flux`, `flux2klein`, `qwen_image`, `anima`. Default **`zimage`**. Purely a starting point — you can switch family per run.
- **`qwen_image`** trains either **Qwen-Image-Edit-2511** (instruction-based editing, the default) or base **Qwen-Image** (text-to-image) — pick the target via the variant selector next to the base picker. Local-only for now (no cloud lane yet). This is a plain training family: for *using* an already-trained multi-angle LoRA to rotate an existing render's camera angle, see **Qwen Multi-angle** under Image engines above instead — that's a separate, unrelated feature. `qwen_image` is also the only family with a choice of **training engine** — **musubi-tuner** (default, if configured under Local tools) or ai-toolkit — picked per run next to the LoRA-type selector. An unset dataset also starts with the built-in **Qwen-Image · Character** preset pre-selected in the presets dropdown (still requires clicking Apply — nothing changes on its own). Any explicit engine/variant/preset choice you've already made always wins over these defaults.
  - Training the **Edit-2511** target (either engine) now auto-generates a **control image** per training image — required by Qwen-Image-Edit's architecture, otherwise the run silently degrades to plain text-to-image and the LoRA never sees the edit-conditioned pass at all. Character, style and concept datasets get a **solid black placeholder** (erases spatial guidance, so the model places the subject/style from the prompt alone instead of memorizing a fixed pose). No setting to configure — this is automatic and only touches the Edit-2511 target; base Qwen-Image (T2I) training is unaffected.
- **`anima`** trains the open [Anima](https://huggingface.co/circlestone-labs/Anima-Base-v1.0-Diffusers) anime model on its public base (no gated download); it is **local-only** for now (needs an up-to-date ai-toolkit + diffusers — cloud training arrives once the GPU pod image is verified).
...
- **Preview images** → `sample_enabled`. Default **off**, every engine (ai-toolkit and musubi-tuner alike). Turning it on renders a test image per prompt at the **Preview every** cadence, so you can watch the LoRA converge and later pick the best checkpoint — but it's an extra decode-to-pixels pass on top of an already tight VRAM budget, and most runs are unattended, so it's opt-in rather than a cost nobody asked for. **Preview every** and **Preview prompts** stay visible but greyed out until it's on, so a run you already configured isn't silently forgotten by flipping the switch back on later.
...
- **Render style** *(Photoreal / 3D render / 2D anime / Semi-real cartoon / Illustration / Custom)* — the target aesthetic for **generated** variations (Nano Banana, ChatGPT, Klein). Not disruptive like kind — nothing is deleted or requires re-captioning, it only affects **future** generations. **Photoreal** is the historical default and is byte-identical to the app's original behaviour; every other value steers the generation prompt toward that look, and on **Klein** it also **skips the consistency LoRA automatically** (it anchors *photographic* composition/structure, which actively fights a stylized target) unless you set its strength by hand in **🖥️ Klein tuning**. Honest expectation: Klein's own model is fine-tuned for photorealistic restaging, so a style direction is best-effort there — the API engines (Nano Banana/ChatGPT) generally follow one more reliably. This setting is orthogonal to **Dataset kind** — any kind can target any render style — and to the curation tools (face scoring, auto-framing, watermark cleanup), which stay geared toward real photos regardless.
