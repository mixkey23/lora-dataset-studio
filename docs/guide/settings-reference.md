...
musubi-tuner runs also generate sample preview images during training (same prompts as ai-toolkit's own sample block), so the Runs-hub card shows a real thumbnail instead of the generic family tile — no setting to configure, this is automatic. Previews render at the training resolution capped at 1024px (never upscaled past a profile's own smaller resolution) — sampling at the full 1328px some GPU profiles train at added a VAE decode spike that could OOM a card already near its budget (reported on a 32GB card using the `rtx5090` profile) without changing the actual training recipe at all.
