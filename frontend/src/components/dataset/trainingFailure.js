...
// musubi-tuner's own first-run causes: a different venv, and Qwen-Image weight
// paths instead of a gated Hugging Face base.
export const MUSUBI_CAUSES =
  'Common first-run causes: musubi-tuner’s Python venv is missing packages '
  + '(re-run its install), one of the 3 Qwen-Image weight paths in Settings is '
  + 'wrong, or the GPU profile’s block-swap/fp8 settings don’t fit this card’s '
  + 'VRAM. Fix the cause above, then Train again.';
...
export function failureView(error) {
  ...
  const rc = error.rc;
  const engineLabel = error.engine === 'musubi' ? 'musubi-tuner' : 'ai-toolkit';
  const title = `The last training run failed${rc != null ? ` (${engineLabel} exited ${rc})` : ''}`
  ...
  let causes = error.engine === 'musubi' ? MUSUBI_CAUSES : GENERIC_CAUSES;
