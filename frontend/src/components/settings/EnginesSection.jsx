...
const ENGINE_OPTIONS = [
  ...
  { id: 'qwen_edit', label: 'Qwen Edit (ComfyUI, local)' },
]
    ...
    >
      ...
      {lightningOn && (
        <label className="flex items-center gap-1.5 text-sm text-content">
          <span className="whitespace-nowrap">Lightning LoRA strength: {light.toFixed(2)}</span>
          <input type="range" min={0} max={1.5} step={0.05} value={light}
            aria-label="Qwen Edit Lightning LoRA strength"
            onChange={(e) => setField('qwen_edit', 'lightning_strength', Number(e.target.value))}
            className="w-40 accent-indigo-500" />
        </label>
      )}
    </Card>
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-border pt-4">
        ...
        <PromptOverrideField
          id="identity-prompt-qwen-edit-identity"
          label="Qwen Edit — restage & face-identity block"
          desc="Same role as the Klein block above, for the local Qwen Edit engine. Steers pose/framing/outfit changes without altering the person. Not per subject type — it applies everywhere."
          rows={3}
          value={ip.qwen_edit_identity}
          defaultText={defaults.qwen_edit_identity}
          onChange={(v) => set('qwen_edit_identity', v)}
          className="mt-3"
        />
      </div>
...
export default function EnginesSection(props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <QwenEditLorasCard config={config} setField={setField} />
