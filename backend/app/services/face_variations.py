...
def _style_aware_identity(kind: str, render_style: str, include_tail: bool = True,
    ...
    `subject_type='human'` (default) -> byte-identical to before this parameter
    existed; the styled base text (_GUARD_BASE_STYLED_BY_KIND/_GUARD_BASE_BY_KIND)
    stays human-phrased regardless — only the override-vs-default comparison and
    the shipped default itself are subject-type-aware."""
    resolved = get_identity_prompt(kind, subject_type)
    if resolved != identity_prompt_default(kind, subject_type):
...
def wrap_variation(prompt: str, ref_count: int = 1, suffix: str = '',
                   render_style: str = 'photoreal', subject_type: str = 'human') -> str:
    ...
    `render_style='photoreal'` and `subject_type='human'` (both defaults) ->
    byte-identical to before either parameter existed; render_style swaps the
    guard's trailing photographic clause for the style's own wording, subject_type
    picks the human/animal/object/creature/other identity lock — see
    _style_aware_identity."""
    guard = _style_aware_identity('face_multi' if ref_count > 1 else 'face_single',
                                  render_style, subject_type=subject_type)
...
def wrap_variation_klein(prompt: str, nsfw: bool = False, framing: str | None = None,
                         suffix: str = '', render_style: str = 'photoreal',
                         subject_type: str = 'human', label: str = '') -> str:
    ...
    3) swaps the medium to 'image' AND replaces the trailing photographic clause
    with the style's own wording (see _compose_edit_prompt) — Klein's own
    fine-tune is still photoreal-biased regardless of prompt, this is
    best-effort steering, not a guarantee.
    `subject_type` picks the human/animal/object/creature/other/anime framing
    detail and identity lock (default 'human' = historical wording).
...
def _compose_edit_prompt(prompt: str, *, nsfw: bool, framing, suffix: str,
    ...
    `render_style='photoreal'` (default) -> the medium/ending stay entirely
    subject_type-driven (byte-identical to before this parameter existed). A
    non-photoreal style with a defined generation_tail swaps the medium to
    'image' and replaces the trailing rendering clause with the style's own
    wording, same as the pre-refactor wrap_variation_klein — this OVERRIDES the
    subject_type medium/tail (Klein's fine-tune is photoreal-biased regardless
    of subject; render_style is the stronger signal when both are non-default)."""
    st = normalize_subject_type(subject_type)
    detail = (get_identity_prompt(f'framing_{framing}', st)
              if framing in PROMPT_FRAMINGS else '').strip()
    style_tail = generation_tail_for(render_style)
    # 'human' is the only noun that reads as photo-specific ('person'); every other
    # subject_type already says something style-neutral (animal/creature/object/
    # subject/character), so only it swaps for a non-photoreal style.
    noun = ('character' if style_tail and st == 'human'
            else _KLEIN_SUBJECT_NOUN.get(st, 'subject'))
    if style_tail:
        medium = 'image'
        ending = (f"Explicit nudity is allowed; render natural, anatomically correct forms. {style_tail}"
                  if nsfw else f"{style_tail} SFW.")
    else:
    ...
    # style_tail truthy -> non-photoreal default in play: `ending` above already
    # carries the style's own tail, so the identity block must not repeat it
    # (include_tail=False). style_tail falsy (photoreal/custom/unknown/an active
    # user override) -> the plain lookup, byte-identical to before this existed —
    # `_style_aware_identity`'s own base-only fallback would silently drop Klein's
    # hardcoded "Sharp focus..." quality tail from the shipped default.
    identity = (_style_aware_identity('klein_identity', render_style, include_tail=False,
                                      subject_type=st)
                if style_tail else get_identity_prompt('klein_identity', st))
    return (
...
QWEN_EDIT_CATALOG_PROMPTS = {
    ...
    'Bust, three-quarter': ('seen from the waist up, turned three-quarters, smiling, wearing an outfit '
                            'distinct from image 1, in a bright modern indoor setting'),
    ...
    'Body sitting': 'in a full-body view, sitting on a chair, a relaxed pose, in a sunlit indoor room',
